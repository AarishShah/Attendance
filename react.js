class attendance {
    ...
    Argument_verifier() {
        if (
            this.Criteria <= 0 || this.Classes_attended < 0 || this.Total_classes_held <= 0 || this.Classes_attended > this.Total_classes_held) {
    ...
    shortage() {
        ...
        if (trigger == 0) {
            document.querySelector(".text2").innerHTML = "The attendance percentage could not reach " + this.Criteria * 100 + "% in next 36500 days."; // This is never occur unless someone is messing around. Test Case: 75/1/30,000
        }
    }
